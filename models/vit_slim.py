...
class Attention(nn.Module):
    def __init__(self, dim, dim1, heads=8, dropout=0.0, qkv_bias=False):
        ...
        """
        self.attn_to_qkv = nn.Linear(dim, dim1 * 3, bias=qkv_bias)
            ...
            for num in cfg:
                self.layers.append(
                    nn.ModuleList(
                        [
                            PreNorm(
                                dim, Attention(dim, num, heads=heads, dropout=dropout)
                            ),
                ...
                self.layers.append(
                    nn.ModuleList(
                        [
                            ...
                            PreNorm(
                                dim, FeedForward(dim, dim, mlp_dim, dropout=dropout)
                            ),
                        ]
                    )
                )
    ...
    def __init__(
        ...
        *,
        ...
        self.transformer = Transformer(
            dim, depth, heads, dim_head, mlp_dim, dropout, cfg
        )
