...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, qkv_bias=False, dropout=0.0):
        super().__init__()
...
class CompressAttention(nn.Module):
    def __init__(
        self, dim, heads=8, qkv_bias=False, dim_head=64, dropout=0.0, reduce=None
    ):
        super().__init__()
        inner_dim = dim_head * heads
        project_out = not (heads == 1 and dim_head == dim)
        ...
        self.attn_dropout = nn.Dropout(dropout)
        self.attn_to_out = (
            nn.Sequential(nn.Linear(inner_dim - 1, dim), nn.Dropout(dropout))
            if project_out
            else nn.Identity()
        )
        self.attn_attend = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        new_q = new_q.reshape(B, N, self.num_heads, 512 // self.num_heads).permute(
            0, 2, 1, 3
        )
        new_k = new_k.reshape(B, N, self.num_heads, 512 // self.num_heads).permute(
            0, 2, 1, 3
        )
        new_v = new_v.reshape(B, N, self.num_heads, 512 // self.num_heads).permute(
            0, 2, 1, 3
        )
