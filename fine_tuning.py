...
def test(epoch):
    ...
    if acc > best_acc:
        print("Saving")
        state = {"net": net.state_dict(), "acc": acc, "epoch": epoch,"cfg": cfg,
                "rate":sum(cfg)/(512*6),}

        if not os.path.isdir("after_tuning"):
            os.makedirs("after_tuning")
        
        method = args.model_path.split("/")[0]

        if not os.path.isdir(f"after_tuning/{method}"):
            os.makedirs(f"after_tuning/{method}")


        torch.save(
            state,
            f"after_tuning/{args.model_path}".format(
                args.patch
            ),
        )
        best_acc = acc
